...
Key translations performed:
  - event_id -> record_id (UUID format enforced)
  - function_id -> function_name (pattern sanitisation applied)
  - ExecutionPhase -> Layer 1 execution phases
  - AnomalyType -> Layer 1 anomaly categories
  - Flat telemetry dataclass -> telemetry_data + context_metadata dicts
  - schema_version v4.2 -> v2.1 at the adapter boundary

Every original Layer 0 field is preserved in either `telemetry_data` or
`context_metadata`.
"""
...
# r-core imports (resolved from r-core repository root)
...
class RCoreToLayer1Adapter:
    """
    Translates a real scafad-r-core TelemetryRecord (v4.2) to a dict that
    satisfies the scafad-delta Layer 1 InputValidationGateway v2.1 schema.

    The adapter is stateless and thread-safe.
    """

    def adapt(self, record: TelemetryRecord) -> Dict[str, Any]:
        """Convert a TelemetryRecord to a Layer 1 v2.1-compatible dict."""
        if record is None:
            raise ValueError("record must not be None")
        if not isinstance(record, TelemetryRecord):
            raise TypeError(
                f"Expected TelemetryRecord, got {type(record).__name__}"
            )
        ...
        # Layer 1 validator treats error_message / error_stack as top-level
        # conditional fields when execution_phase == "error".
        if phase_str == "error":
