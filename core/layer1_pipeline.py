"""Canonical Layer 1 intake pipeline for SCAFAD in r-core.

This module promotes the previously test-proven Layer 1 gateway behavior into a
real implementation path inside the primary dissertation repository.
"""
...
class Layer1AuditRecord:
    """Audit summary for canonical Layer 1 processing."""
...
class Layer1QualityReport:
    """Lightweight quality report for the post-L1 record."""
...
class Layer1ProcessedRecord:
    """Canonical processed-record contract exiting Layer 1."""
...
class Layer1CanonicalPipeline:
    """Canonical Layer 1 processing path owned by r-core."""
    ...
    def process_l0_record(self, record: TelemetryRecord) -> Layer1ProcessedRecord:
        ...
        return self.process_adapted_record(adapted)
    ...
    def _validate_shape(self, record: Dict[str, Any]) -> None:
        ...
        if missing:
            raise ValueError(f"Adapted record missing required fields: {missing}")
        if not isinstance(record["telemetry_data"], dict):
            raise TypeError("telemetry_data must be a dict")
